...
export default function DropDown({ onSelectSortOption }: DropDownProps) {
  const handleChange = (e: ChangeEvent<HTMLSelectElement>) => {
    onSelectSortOption(e.target.value as 'time' | 'pay' | 'hour' | 'shop'); // 선택한 옵션을 부모 컴포넌트에 전달
  };

  return (
    <select className={cx('dropDown')} id="dropdown" onChange={handleChange}>
      <option value="time">시간적은순</option>
      <option value="pay">시급많은순</option>
      <option value="hour">마감임박순</option>
